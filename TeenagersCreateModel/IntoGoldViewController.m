...
//绑定银行卡
    ...
    UILabel *zjpassWordBankLable = [[UILabel alloc] initWithFrame:CGRectMake(10, 184 + 40, 80, 15)];
    zjpassWordBankLable.font = [UIFont systemFontOfSize:15];
    zjpassWordBankLable.textAlignment = NSTextAlignmentRight;
    zjpassWordBankLable.text = @"资金密码:";
    [self.view addSubview:zjpassWordBankLable];
    
     zjpassWordTF = [[UITextField alloc] initWithFrame:CGRectMake(95, 184 - 7.5 + 40, ScreenWidth - 95 - 20 , 30)];
     zjpassWordTF.font = [UIFont systemFontOfSize:11];
     zjpassWordTF.backgroundColor = [UIColor whiteColor];
     zjpassWordTF.borderStyle = UITextBorderStyleLine;
     zjpassWordTF.placeholder = @"请输入资金密码";
     zjpassWordTF.clearButtonMode = UITextFieldViewModeAlways;
     zjpassWordTF.secureTextEntry = YES;
     zjpassWordTF.delegate = self;
    [self.view addSubview: zjpassWordTF];
    ...
    commitBtn.frame = CGRectMake(50, 284, ScreenWidth - 100, 50);
...
-(void)commitBtnMethods:(id)sender {
    ...
    if ([bankName isEqualToString:@""]) {
    ...
    }else if ([zjpassWordTF.text isEqualToString:@""]) {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_LOW, 0), ^{
            [self requestCategoryList:bankName yhzh:intoBankTF.text yhmm:bankpassWordTF.text zjmm:zjpassWordTF.text tag:kBusinessTagGetBankRJ];
...
//获取品牌列表
- (void)requestCategoryList:(NSString *)yhdm yhzh:(NSString *)yhzh  yhmm:(NSString *)yhmm zjmm:(NSString *)zjmm tag:(kBusinessTag)_tag
{
